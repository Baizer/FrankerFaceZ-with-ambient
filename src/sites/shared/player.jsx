...
export default class PlayerBase extends Module {
	...
	registerSettings() {
		...
		this.settings.add('player.auto-ambient', {
			default: false,
			ui: {
				sort: -1,
				path: 'Player > General >> Ambient Mode @{"sort": 500}',
				title: 'Enable Ambient Mode.',
				description: 'Projects the colors from the edges of the video onto the surrounding area.',
				component: 'setting-check-box'
			},

			changed: () => {
				for(const inst of this.Player.instances)
					this.enableAmbientMode(inst);
			}
		});

		this.settings.add('player.ambient.intensity', {
			default: 85,
			requires: ['player.auto-ambient'],
			process(ctx, val) {
				if ( ! ctx.get('player.auto-ambient') )
					return 85;
				return val;
			},
			ui: {
				path: 'Player > General >> Ambient Mode',
				title: 'Intensity',
		...
		this.settings.add('player.ambient.width', {
			default: 50,
			requires: ['player.auto-ambient'],
			process(ctx, val) {
				if ( ! ctx.get('player.auto-ambient') )
		...
		this.settings.add('player.ambient.invert', {
			default: false,
			requires: ['player.auto-ambient'],
			process(ctx, val) {
				if ( ! ctx.get('player.auto-ambient') )
	...
	async onEnable() {
		...
		this.css_tweaks.set('player-ambient-base', `
			.ffz-ambient-bg {
				position: absolute;
				inset: 0;
				z-index: 0;
	...
	enableAmbientMode(inst) {
		const enabled = this.settings.get('player.auto-ambient');
		...
		const update = timestamp => {
			...
			try {
				const intensity     = (this.settings.get('player.ambient.intensity') ?? 85) / 100;
				const widthSetting  = this.settings.get('player.ambient.width') ?? 50;
				const w             = widthSetting / 2;
				const invert        = this.settings.get('player.ambient.invert');
